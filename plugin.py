...
legacy_userdb = DictDB(os.path.join(conf.supybot.directories.data(), 'users.pklz'), mode=0o600)
...
def normalReply(self, irc, msg, to):
	irc.reply(msg, to)

def specialReply(self, irc, msg, to):
	if(len(mod) >= 2 and len(mod[1]) > 2 and
		mod[1][0] == mod[1][1] and
		mod[1][0] in conf.supybot.reply.whenAddressedBy.chars()):
		irc.reply(msg, private=True, notice=True)
	else:
		normalReply(self, irc, msg, to)
...
class Tag(object):
	...
	def getInfo(self):
		...
		return {'summary': str(summary, 'utf-8', 'ignore'),
				'content': str(content, 'utf-8', 'ignore')}
...
class Track(object):
	...
	def getInfo(self, username, autocorrect=1):
		...
		return Track(track_elem.find('name').text,
			artist=Artist(track_elem.find('artist').find('name').text),
...
class User(object):
	...
	def getRecentTracks(self, start=None, end=None, limit=None, page=None):
		...
		return [Track(name=track_elem.find('name').text,
						artist=Artist(name = track_elem.find('artist').find('name').text),
						...
						loved=track_elem.find('loved').text
						)
...
def build_url(url, extra_params):
	...
	if extra_params and len(extra_params) > 0:
		...
		extra_query = urllib.parse.urlencode(
						[(k, str(extra_params[k]).encode('utf-8'))
...
def fetch(url_base, key, args):
	args = args or dict()
	url = build_url(url_base, dict(list(key.items()) + list(args.items())))
	print(url)
...
def topTagsFromChart(target, start=None, end=None):
	...
	for chart in charts:
		...
		for artist in chart.artists[:artist_count]:
			...
			tag_count = 0
			weighted_tfidfs = {}
			if artist.name not in list(artist_top_tags.keys()):
				artist_top_tags[artist.name] = find_artist(artist.name).tags
...
def hostmask_clean(mask):
	return functools.reduce(lambda acc, x: acc.replace(x, '_'),
					['!','@','.','~'],
...
class Lastfm(callbacks.Plugin):
	...
	users = DictDB(os.path.join(conf.supybot.directories.data(), 'users.pklz'), mode=0o600)
	...
	def trelleborg(self, irc, msg, args):
		self.reply(irc, immigrant_song[self.viking_line % len(immigrant_song)], None, None)
		self.viking_line += 1
...
#*******************************************************************************
	...
	def unbantag(self, irc, msg, args, tag):
		...
		try:
			if ircdb.checkCapability(msg.prefix, 'owner'):
				unban_tag(tag)
				self.reply(irc, "'%s' removed from banned tags" % (tag), None)
			else:
				self.reply(irc, "No permissions for you peon!", None)
		except Exception as e:
			self.reply(irc, str(e), None)
	...
	def bantag(self, irc, msg, args, tag):
		...
		try:
			if ircdb.checkCapability(msg.prefix, 'owner'):
				ban_tag(tag)
				self.reply(irc, "'%s' added to banned tags" % (tag), None)
			else:
				self.reply(irc, "No permissions for you peon!", None)
		except Exception as e:
			self.reply(irc, str(e), None)
	...
	def tagrank(self, irc, msg, args, tag):
		...
		try:
			...
			self.reply(irc, "no tags found", None)
			return
	...
	def multitagged(self, irc, msg, args, text):
		...
		for t in reversed(tags):
			artists = [a for a in artists if gettag(a, t).count > 10]
			...
			for n in neighbours:
				...
				for a in artists:
					if a.name in my_artists:
						continue
					if a.name in list(global_artists.values()):
	...
	def whatsplaying_thread(self, irc, msg, args):
		...
		for n in nicks:
			...
			try:
				...
				now_playing = True if track[0].played_on > timelimit else False\
				...
				if now_playing:
					...
					tag_str = ""
					if tags:
						tag_str = "(%s)" % ', '.join([t.name.decode('utf-8') for t in tags[:3]])
					out = "[%s.playing]: %s - %s	%s" % (n, track[0].artist.name, \
								track[0].name, tag_str)
					self.reply(irc, out, None)
					hits += 1
		...
		for user in users:
			try:
				...
				if track and track[0]:
					...
					tag_str = ""
					if tags:
						tag_str = "({})".format(', '.join([t.name.decode('utf-8') for t in tags[:3]]))

					loved = "♥" if track[0].loved == '1' else ""
					playcount = "[{} plays]".format(track[0].stats.userplaycount) if track[0].stats.userplaycount != 0 else ""
					print(track[0].stats.userplaycount)
					result = "%s - %s	%s %s %s" % (track[0].artist.name, track[0].name, tag_str, playcount, loved)
				else:
					result = "nothing"
				out = "%s %s" % (out, result)
			except LastfmError as e:
				out = error_msg(msg, e)
			self.reply(irc, out, channel)
			...
			for a in taste.artists:
				for t in filter_tags(a.tags)[:5]:
					if t.name not in ignore_tags:
						tags[t.name] += 1
			tags = sorted(iter(tags.items()), key=operator.itemgetter(1), reverse=True)

			out = "[%s.compare.%s]: %s (%.2f%%) %s feat. %s" % \
					(left.name, right.name, \
					 taste.score_name(), taste.stats.score * 100, \
					 ', '.join(['%s' % t[0].decode('utf-8') for t in tags[:3] if tags]), \
	...
	def heard(self, irc, msg, args, account, artist):
		...
		try:
			...
			if len(taste.artists) == 0:
				...
				found = ', '.join([t.name.decode('utf-8') for t in taste.artists])
			else:
